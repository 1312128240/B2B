...
public class GoodsFragment extends MyBaseFragment implements GoodsXqInterface,ScollListener{
    ...
    private void initEvaluate(GoodsXqBean.DataBean.CommentBean comment) {
          tv_ev_num.setText("客户评价   ("+comment.getEvalutesum().getTs()+")"  );
          tv_hpl.setText(Html.fromHtml("好评率 "+"<font color='#ff0000'>" + comment.getEvalutesum().getHpl()+ "</font>"));
        List<GoodsXqBean.DataBean.CommentBean.EvalutesumBean> evList= (List<GoodsXqBean.DataBean.CommentBean.EvalutesumBean>) comment.getEvalute();
        /*  CommonAdapter<GoodsXqBean.DataBean.CommentBean.EvalutesumBean> EvAdapter=
                  new CommonAdapter<GoodsXqBean.DataBean.CommentBean.EvalutesumBean>(MyApp.getContext(),R.layout.ev_item,) {
              @Override
              protected void convert(ViewHolder holder, GoodsXqBean.DataBean.CommentBean.EvalutesumBean evalutesumBean, int position) {

              }
          };
          recy_ev.setAdapter(EvAdapter);*/
        Log.i("评论大小是",evList.size()+"");
    }
